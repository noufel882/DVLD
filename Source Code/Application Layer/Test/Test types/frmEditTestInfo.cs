...
namespace Application_Layer.Test_types
{
    public partial class frmEditTestInfo : Form
    {
        ...
        private TestType.enTestTypes _ID = TestType.enTestTypes.VisionTest;
        private TestType _type;

        public frmEditTestInfo(int ID)
        {
            InitializeComponent();
            _ID = (TestType.enTestTypes)ID;
        }

        private void FillAppTypeInfo()
        {
            lblID.Text = ((int)_ID).ToString();
            txtTitle.Text = _type.Title;
            txtFees.Text = Convert.ToString(_type.Fees);
            txtDescription.Text = _type.Description;
        }

        private void _LoadData()
        {
            _type = TestType.Find(this._ID);

            if (_type == null)
            {
                MessageBox.Show("This Test type is no longer exist .", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                this.Close();
                return;
            }
        ...
        private void txtDescription_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                errorProvider1.SetError(txtDescription, "This field is require !");
                return;
            }
            else
            {
        ...
        private void txtFees_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                errorProvider1.SetError(txtFees, "Fill this blank !");
                return;
            }
        ...
        private void SetDataToSave()
        {
            ...
            _type.Description = txtDescription.Text;
        }
